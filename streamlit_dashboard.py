...
# FIX #7: Deduplicated NIFTY500 list (duplicates removed at source)
NIFTY500 = [
    ...
    "AFFLE","SCHAEFFLER","TIMKEN","GRINDWELL","TIINDIA",
    "IRCON","NBCC","ENGINERSIN","CONCOR","JSWINFRA",
    "JIOFIN","BAJAJHLDNG","CHOLAHLDNG","MFSL","PIRAMALFIN",
]
NIFTY500 = list(dict.fromkeys(NIFTY500))  # safety dedup preserving order
...
def fetch_data(symbol_ns, period="2y"):
    ...
    Post-market cache (after 3:30 PM IST) is treated as final for the day.
    Falls back to any existing cache if download fails.
    """
    cache = f"data_cache/{symbol_ns.replace('.','_').replace('^','X').replace('&','_')}.csv"

    now_ist          = datetime.datetime.utcnow() + datetime.timedelta(hours=5, minutes=30)
    today            = now_ist.date()
    market_close_utc = datetime.datetime.combine(today, datetime.time(10, 0))  # 3:30 PM IST = 10:00 UTC
    ...
    if os.path.exists(cache):
        ...
        cache_post_close = mtime > market_close_utc
...
# FIX #8: RS score now uses outperformance (difference), not ratio with abs()
def compute_rs_score(stock_df, bench_df):
    """
    Composite RS Score vs Nifty 500 benchmark.
    RS Score = average of:
      (stock 3M return - benchmark 3M return)
      (stock 6M return - benchmark 6M return)
...
def read_closed_from_sheet():
    ...
    # FIX #10: Show a visible warning on error instead of silent []
    try:
        df = pd.read_csv(CLOSED_URL)
        df.columns = [c.strip().lower().replace(" ", "_") for c in df.columns]

        if "exit_price" not in df.columns:
            return []  # Genuinely empty closed tab
...
def get_signal_state(df):
    """
    4-step signal state machine:

    Step 1 — EMA Cross (Day 0):
      Close crosses above EMA 220 from below.
      Lock w52_fixed = max(Close) for 365 calendar days BEFORE Day 0.
    ...
    Step 3 — Breakout confirmation window (10 trading days after 52W break).

    Step 4 — Confirmed: 10+ days since 52W break, still above EMA 220.

    Returns: (state, info_dict)
      state = 'none' | 'ema_cross' | 'watching' | 'near_52w' | 'breakout' | 'confirmed'
    """
    # FIX #9: Guard against dataframes with fewer than 2 usable rows
    ...
    # ── Search for most recent EMA cross within last 60 trading days ──
    cross_idx  = None
    w52_fixed  = None
    cross_date = None
    ...
    # FIX #1 & #2: Always initialise days_since_break before branching,
    # and properly implement the "breakout" state as a 10-day confirmation window.
    days_since_break = 0
    break_idx        = None
    ...
    if days_since_cross <= 1:
        ...
        # FIX #2: "breakout" = in 10-day window, "confirmed" = 10+ days elapsed
    ...
    info = {
        ...
        "days_since_break": days_since_break,  # FIX #1: always defined
...
def build_chart(df, symbol, show_days=180):
    # FIX #9: Guard against too-short dataframes
...
with st.sidebar:
    ...
    # FIX #4: Two separate refresh buttons so price re-downloads don't nuke sheet caches
    col_r1, col_r2 = st.columns(2)
    with col_r1:
        if st.button("🔄 Prices", use_container_width=True, help="Re-download price data from yfinance"):
            fetch_data.clear()
            st.rerun()
    with col_r2:
        if st.button("📋 Sheet", use_container_width=True, help="Reload positions from Google Sheet"):
            read_positions_from_sheet.clear()
            read_closed_from_sheet.clear()
            st.rerun()
...
if page == "📊 Stock Chart":
    ...
    if symbol:
        ...
        if df_raw is None or df_raw.empty:
            ...
            cmp = float(row["Close"]); chg = info.get("change_pct", 0)

            m1, m2, m3, m4, m5, m6 = st.columns(6)
            with m1: st.metric("CMP", f"₹{cmp:,.2f}", f"{chg:+.2f}%")
            with m2: st.metric("EMA 220", f"₹{info['ema220']:,.2f}",
                               f"{info['pct_above_ema']:+.2f}% {'above' if info['above_ema'] else 'below'}")
            with m3: st.metric("52W High", f"₹{info['w52_high']:,.2f}", f"{info['pct_from_52w']:+.2f}%")
            with m4: st.metric("RSI (14)", f"{info['rsi']}")
            with m5: st.metric("SL Level", f"₹{info['sl_level']:,.2f}")
            with m6: st.metric("Vol (20d avg)", f"{info['vol20']/1e5:.1f}L")

            state_map = {
                "confirmed": ("✅ CONFIRMED — Strategy says: Trade ready next day!", "badge-green"),
            ...
            if cls:
                st.markdown(f'<div class="signal-badge {cls}">{label}</div><br>', unsafe_allow_html=True)
            ...
            with c1:
                levels = pd.DataFrame({
                    "Level": ["Current Price", "EMA 220", "EMA 50", "EMA 20",
                              "52W High", "SL (max of EMA220, -10%)"],
                    "Value (₹)": [
                        f"{cmp:,.2f}", f"{info['ema220']:,.2f}",
            ...
            with c2:
                targets = pd.DataFrame({
                    "Target": ["+20%", "+40% (sell 50%)", "+100% (sell 25%)", "+150%", "+200%"],
                    "Price (₹)": [
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if is_weekend:
        st.warning(
            ...
            "Signals on weekends may be stale. Run on Monday after 3:30 PM IST for accurate results."
        )
        ...
        st.warning(
            ...
            "Scan after 3:30 PM IST when today's close is final."
        )
    elif is_post_close:
        st.success(
    ...
    if last_scan_time and not is_weekend and is_post_close:
        try:
            ...
            if scan_dt.date() == now_ist.date() and scan_dt.time() > market_close:
                scan_locked = True
        except Exception:
            pass
    ...
    if scan_locked:
        st.info(
            f"🔒 **Scan results locked for today ({now_ist.strftime('%d %b %Y')}).**\n\n"
            "Already ran a post-market scan today. Results frozen to prevent signal flipping."
        )
    ...
    if st.button(scan_label, type="primary", use_container_width=True):
        if is_weekend:
            st.warning("Running scan on weekend — treat results with caution.")
        if scan_locked:
            st.warning("⚠️ Re-scanning will overwrite today's locked results.")
    ...
    if "scan_results" in st.session_state:
        ...
        tag_icon = {
            ...
            "weekend"     : "⚠️ Weekend scan",
            "market_hours": "⚡ During market hours",
        }.get(scan_tag, "")
        st.caption(
            f"Last scan: {st.session_state.get('scan_time','')} · "
            f"{tag_icon} · {len(NIFTY500)} stocks checked · {len(results)} signals found"
        )
        ...
        if not filtered:
            ...
            for r in filtered:
                ...
                rows.append({
                    ...
                    "Days since 52W break": r.get("days_since_break", "—"),  # FIX #1
                })
        ...
        if debug_sym:
            ...
            if df_dbg is None:
                ...
                st.markdown("**Last 20 days — Close vs EMA 220:**")
    ...
    if not positions:
        ...
        with st.spinner("Fetching prices…"):
            for pos in positions:
                ...
                if df_raw is None or len(df_raw) < 2:   # FIX #9
                    continue
                ...
                live.append({
                    "symbol": sym, "entry_price": ep, "shares": sh,
                    "entry_date": estr, "hold_days": days,
                    "cmp": round(cmp, 2), "ema220": round(ema, 2),
                    "pnl_pct": round(pct, 2), "pnl_rs": round((cmp - ep) * sh, 2),
                    "change_pct": round(chg, 2), "trailing_sl": round(sl, 2),
        ...
        if not live:
            ...
            def _card(label, value, sub="", sub_color="#888"):
                return (
                    f'<div style="background:#1e1e30;border:1px solid #4a4a70;border-radius:10px;'
                    f'padding:14px 16px;min-width:0;flex:1;">'
                    f'<div style="font-size:12px;color:#c8c8e8;margin-bottom:6px;white-space:nowrap;">{label}</div>'
            ...
            for p in live:
                if p["near_sl"]:    st.error(f"⚠️ {p['symbol']} near SL ₹{p['trailing_sl']}")
            ...
            for p in sorted(live, key=lambda x: x["pnl_pct"], reverse=True):
                ...
                rows.append({
                    "Stock": p["symbol"], "Qty": p["shares"],
                    "Buy Price": f"₹{p['entry_price']:,.2f}", "CMP": f"₹{p['cmp']:,.2f}",
                    "Invested": f"₹{inv:,.0f}", "Current": f"₹{p['cmp']*p['shares']:,.0f}",
                    "Day %": f"{p['change_pct']:+.2f}%", "Total P&L": f"₹{p['pnl_rs']:+,.0f}",
                    "Total %": f"{p['pnl_pct']:+.2f}%", "SL": f"₹{p['trailing_sl']:,.0f}",
                    "Days": p["hold_days"],
                })
            ...
            for htab, p in zip(htabs, live):
                with htab:
                    ...
                    m3.metric("Total P&L",    f"₹{p['pnl_rs']:+,.0f}",  f"{p['pnl_pct']:+.2f}%")
                    ...
                    with c1:
                        ...
                        st.metric("Current Value", f"₹{curr:,.0f}", f"₹{p['pnl_rs']:+,.0f}")
                    with c2:
                        st.markdown("**P&L**")
                        st.metric("Today Change", f"{p['change_pct']:+.2f}%")
                        st.metric("Today P&L",    f"₹{day_p:+,.0f}")
                        st.metric("Total P&L",    f"₹{p['pnl_rs']:+,.0f}", f"{p['pnl_pct']:+.2f}%")
                    with c3:
            ...
            with st.expander("Log a Closed Trade"):
                ...
                with lc2: ct_ep     = st.number_input("Entry ₹",  min_value=0.0, step=0.5, key="ct_ep")
                with lc3: ct_xp     = st.number_input("Exit ₹",   min_value=0.0, step=0.5, key="ct_xp")
                ...
                ct_reason = st.selectbox("Reason",
                    ["Trailing SL", "+40% Profit", "+100% Profit", "Manual Exit"], key="ct_reason")
            ...
            if closed:
                ...
                loss_sum = sum(t["pnl_rs"] for t in losses)
                pf       = sum(t["pnl_rs"] for t in wins) / abs(loss_sum) if loss_sum != 0 else 0
                ...
                trows = [{
                    "Symbol": t["symbol"], "Entry Date": t["entry_date"], "Exit Date": t["exit_date"],
                    "Entry ₹": t["entry_price"], "Exit ₹": t["exit_price"], "Shares": t["shares"],
                    "P&L ₹": f"₹{t['pnl_rs']:+,.0f}", "P&L %": f"{t['pnl_pct']:+.2f}%",
                    "Days": t["hold_days"], "Reason": t["reason"],
            ...
            with st.expander("How to Add Positions"):
                st.markdown(
                    f"All position management in [Google Sheet]({get_sheet_link()}). "
                    "Add rows in Positions tab. Delete and move to Closed tab when exiting."
                )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.spinner("Loading benchmark data..."):
        df_n50  = fetch_data("^NSEI",   "5y")
        df_n500 = fetch_data("^CRSLDX", "5y")
        if df_n500 is None or df_n500.empty:
            df_n500 = fetch_data("^NSEI", "5y")

    idx_tab = st.radio("Select Index", ["Nifty 50", "Nifty 500", "Compare Both"], horizontal=True)
    ...
    if idx_tab == "Nifty 50":
        ...
        if df_n50 is None or df_n500 is None or len(df_n50) < 2 or len(df_n500) < 2:  # FIX #9
            st.error("Could not load sufficient data for comparison.")
        else:
            df50  = add_indicators(df_n50);  df500 = add_indicators(df_n500)
            ...
            for col, (label, n) in zip(cols, periods_cmp):
                if n is None:
                    v50 = f"{chg50:+.2f}%"; v500 = f"{chg500:+.2f}%"
                else:
                    r50v = ret_idx(df50, n); r500v = ret_idx(df500, n)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("Calculates exact shares and capital to deploy using the 1% risk rule.")

    st.markdown("### Enter Trade Details")
    c1, c2 = st.columns(2)
    with c1:
        capital = st.number_input("Current Capital (₹)",
            min_value=10000, max_value=10000000, value=500000, step=5000,
            help="Your total available trading capital right now")
    with c2:
        sym_input = st.text_input("NSE Symbol (auto-fetches CMP & EMA 220)",
            placeholder="e.g. AARTIIND", value="").upper().strip()

    manual = st.toggle("Enter price manually instead of auto-fetch", value=False)

    if manual or not sym_input:
        c1, c2 = st.columns(2)
        with c1: entry_price = st.number_input("Entry Price ₹", min_value=1.0, step=0.5, value=500.0)
        with c2: ema220_val  = st.number_input("EMA 220 ₹",     min_value=1.0, step=0.5, value=430.0)
    else:
        ...
        entry_price = st.number_input(
            ...
            min_value=1.0, step=0.5, value=round(cmp_live, 2),
            help="Strategy enters at next day's open price.")
    ...
    if capped:
        st.warning(f"⚠️ Position capped at 20%. Original shares: {math.floor(risk_amount/risk_per_sh)}, capped to {shares}.")
    ...
    with r2:
        ...
        st.dataframe(pd.DataFrame({
            ...
            "Value": [f"₹{capital:,.0f}","1%",f"₹{risk_amount:,.0f}",
                      f"₹{risk_per_sh:,.2f}",f"{shares_raw:.1f}",f"{shares} (floor)"],
        }), hide_index=True, use_container_width=True)
    with r3:
        ...
        st.dataframe(pd.DataFrame({
            ...
            "Status"  : ["✅" if check_min else "❌","✅" if check_max else "❌","✅" if check_valid else "❌"],
        }), hide_index=True, use_container_width=True)
    ...
    t4.metric("Remaining 25% held",f"{math.ceil(shares*0.25)} shares", "Hold until trailing SL")

    st.markdown("---")
    st.markdown("#### 📅 Trailing SL Schedule")
    st.caption("Updated every Friday. New SL = max(EMA 220, Friday close × 0.90). Only moves UP.")
    trail_rows = []
    ...
    batch_syms = st.text_input("Enter symbols separated by commas",
        placeholder="AARTIIND, KAYNES, LUPIN, SUNPHARMA", value="")
    ...
    if batch_syms and st.button("Calculate All", type="primary"):
        ...
        with st.spinner("Fetching prices..."):
            for s in symbols_list:
                ...
                if df_b is None or len(df_b) < 2:   # FIX #9
                ...
                total_deploy += dep_b
                batch_rows.append({
                    "Symbol"    : s, "CMP ₹": f"₹{cb:,.2f}", "EMA 220 ₹": f"₹{eb:,.2f}",
